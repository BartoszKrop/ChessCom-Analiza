...
st.markdown("""
    ...
    /* Centrowanie logowania na mobile */
    .login-box {
        display: flex;
...
def fetch_data(user):
    try:
        headers = {"User-Agent": f"ChessApp-MobileFix-{user}"}
...
# --- EKRAN LOGOWANIA (GŁÓWNY, JEŚLI BRAK DANYCH) ---
if st.session_state.data is None:
    st.title("♟️ Chess Analytics")
    st.write("Wpisz swój nick, aby przeanalizować gry:")
        ...
        if nick:
            with st.spinner("Pobieranie danych..."):
                ...
                if fetched[2] is not None and not fetched[2].empty:
                    ...
                    st.error("Nie znaleziono użytkownika lub gier.")
        else:
    ...
    # Sidebar tylko do zmiany gracza
    with st.sidebar:
    ...
    if not df_f.empty:
        ...
        with tab1:
            ...
            for mode in ["Rapid", "Blitz", "Bullet"]:
                ...
                if not m_df.empty:
                    st.plotly_chart(px.line(m_df, x="Timestamp", y="Elo_Moje", title=f"Trend {mode}"), use_container_width=True)
        ...
        with tab2:
            ...
            day_df["Data_F"] = pd.to_datetime(day_df["Data"]).dt.strftime("%d/%m")
            day_df["Bilans"] = day_df.apply(lambda x: f"{int(x['W'])}/{int(x['R'])}/{int(x['P'])}", axis=1)
            day_df["ELO"] = day_df["E"].round(0).astype(int)
            st.dataframe(day_df[["Data_F", "T", "Bilans", "ELO"]].rename(columns={"Data_F":"Dzień", "T":"Gry"}).sort_values("Dzień", ascending=False), use_container_width=True, hide_index=True)
