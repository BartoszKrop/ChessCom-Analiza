...
st.markdown("""
    ...
    /* Stylizacja multiselect na półprzezroczysty niebieski zamiast czerwonego */
...
if st.session_state.data is None:
    ...
    # Header i Zębatka
    c_logo, c_main, c_gear = st.columns([1, 5, 2])
    with c_logo:
        if profile.get("avatar"): 
            st.markdown(f'<img src="{profile.get("avatar")}" style="border-radius: 10px; width: 50px; margin-top: 15px;">', unsafe_allow_html=True)
    with c_main:
        st.markdown(f"<h4 style='margin-bottom: 0px; margin-top: 10px;'>ChessStats</h4><div style='color: #8b949e; font-size: 0.9rem;'>{username}</div>", unsafe_allow_html=True)
        
    with st.expander("⚙️ Ustawienia i Filtry"):
        rc1, rc2 = st.columns(2)
        if rc1.button("Odśwież", use_container_width=True):
            fetch_data.clear() 
            ...
            st.rerun()
        if rc2.button("Zmień gracza", use_container_width=True):
        ...
        app_m = st.radio("Tryb działania:", ["👤 Moja Analiza", "⚔️ Porównanie Graczy"], horizontal=True)
        
        st.write("---")
        f1, f2, f3 = st.columns(3)
        d_r = f1.date_input("Zakres:", value=(df["Data"].min(), df["Data"].max()))
        s_m = f2.selectbox("Tryb:", ["Wszystkie"] + sorted(df["Tryb"].unique().tolist()))
        s_c = f3.multiselect("Kolor:", ["Białe", "Czarne"], default=["Białe", "Czarne"])
        
        fa1, fa2 = st.columns(2)
        s_h = fa1.slider("Godziny:", 0, 23, (0, 23))
        
        s_a = df["Konto"].unique().tolist()
        if len(df["Konto"].unique()) > 1:
            s_a = fa2.multiselect("Konto:", df["Konto"].unique().tolist(), default=df["Konto"].unique().tolist())
    ...
    if app_m == "👤 Moja Analiza":
        ...
        if not df_f.empty:
            ...
            with t1:
                ...
                st.write("---")
                p1, p2 = st.columns(2)
                p1.plotly_chart(px.pie(df_f[df_f["Wynik"]=="Wygrane"], names="Powod", hole=0.4, title="Jak wygrywam?", color_discrete_sequence=px.colors.sequential.Teal), use_container_width=True)
                p2.plotly_chart(px.pie(df_f[df_f["Wynik"]=="Przegrane"], names="Powod", hole=0.4, title="Jak przegrywam?", color_discrete_sequence=px.colors.sequential.Reds), use_container_width=True)
        ...
        st.write("Wybierz rywala, z którym chcesz się porównać:")
        c1, c2 = st.columns(2)
        p2 = c1.selectbox("Platforma rywala:", ["Chess.com", "Lichess"])
        ...
        if st.session_state.data2 is not None:
            ...
            if isinstance(d_r, (list, tuple)) and len(d_r) == 2:
                df1_c = df1_c[(df1_c["Data"] >= d_r[0]) & (df1_c["Data"] <= d_r[1])]
                df2_c = df2_c[(df2_c["Data"] >= d_r[0]) & (df2_c["Data"] <= d_r[1])]
                
            df1_c = df1_c[df1_c["Kolor"].isin(s_c) & (df1_c["Godzina"] >= s_h[0]) & (df1_c["Godzina"] <= s_h[1])]
            df2_c = df2_c[df2_c["Kolor"].isin(sc_c) & (df2_c["Godzina"] >= s_h[0]) & (df2_c["Godzina"] <= s_h[1])]
            
            if s_m != "Wszystkie":
                df1_c = df1_c[df1_c["Tryb"] == s_m]
                df2_c = df2_c[df2_c["Tryb"] == s_m]
